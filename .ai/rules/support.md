...
## Public verify page holder comes from order or approved claim
`maison.verify` authenticates an `EditionPiece` by `verification_token`. Holder payload resolves from the linked order (`order.name` + `order.user`) or, when `order_id` is null (claim-only allocation), the latest approved `FoundingCircleClaim` user. Do not expose email. Pass Dutch `statusLabel` keys for frontend `t()`, localize dates with `translatedFormat`, and keep `productName` on `product->translated('name')`.
